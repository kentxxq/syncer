...
import type { Package, SyncConfig, SyncStatus, TargetStatus, PackageStatus } from '../../shared/types'
...
/** 检查单文件包某个目标的同步状态 */
function checkFileTargetStatus(
  pkg: Package,
  targetPath: string,
  config: SyncConfig
): SyncStatus {
...
export function executeSync(pkgName?: string): void {
  const config = loadConfig()
  const packages = pkgName
    ? config.packages.filter((p) => p.name === pkgName)
    : config.packages
